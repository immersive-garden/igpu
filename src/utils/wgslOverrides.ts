...
function formatValue(value: number | boolean, type: string | undefined): string {
...
function tryEvalExpr(expr: string): number | undefined {
...
function resolveExpr(expr: string, resolved: Record<string, number>): string {
    let result = expr;
...
export function applyOverrideConstants(code: string, constants: Record<string, number> = {}): string {
    ...
    return code.replace(OVERRIDE_RE, (match, name: string, type: string | undefined, def: string | undefined) => {
