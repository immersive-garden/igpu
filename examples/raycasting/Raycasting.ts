...
type HitMesh = Mesh & { isHit: boolean };
...
export class Raycasting {
    ...
    async init() {
        ...
        const makeMesh = (label: string, geometry: any, position: number[] | null): HitMesh => {
            ...
            if (position) mesh.position.set(...(position as [number, number, number]));
            // share a pipeline but keep per-mesh hit state: write uHit just before draw
            mesh.onBeforeRender((({ mesh }: { mesh: HitMesh }) => mesh.uniforms.set({ uHit: mesh.isHit ? 1 : 0 })) as any);
        ...
        (sphere.geometry as any).raycast = 'sphere';
    ...
    handleMove = (e: MouseEvent) => {
        ...
        this.raycast.castMouse(this.camera, ndc as [number, number]);
        this.meshes.forEach((m) => (m.isHit = false));
        (this.raycast.intersectBounds(this.meshes) as HitMesh[]).forEach((m) => (m.isHit = true));
    };
