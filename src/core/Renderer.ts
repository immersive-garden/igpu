...
import type { GPU, ClearColor } from '@/types';
...
export class Renderer {
    ...
    renderQueue?: Mesh[];
    callBacks: Set<(args: UpdateArgs) => void>;
    deviceLostHandlers: Set<(info: GPUDeviceLostInfo) => void>;
    deviceRestoredHandlers: Set<(gpu: GPU) => void>;
    resizeHandlers: Set<(w: number, h: number) => void>;
    ready: ReturnType<typeof getPromise>;
    isReady: boolean;
    _initialized: boolean;
    _bootPromises: Promise<any>[];
    _bootStarted: boolean;
    bootProgressHandlers: Set<(pct: number) => void>;
    bootCompleteHandlers: Set<() => void>;
    gpuAverage?: NonNegativeRollingAverage;
    ...
    _debug?: boolean;
    _bootSettled?: number;
    _bootProgress?: number;
    _bootComplete?: boolean;
    _gpuMs?: number;

    constructor({ canvas = null, dpr = null, transparent = false, depth = true, stencil = true }: RendererOptions = {}) {
    ...
    async initDevice() {
        ...
        if (!window.ktx) {
            ...
            await window.createKtxReadModule!({
                locateFile: (p: string) => (p.endsWith('.wasm') ? `${import.meta.env.BASE_URL}libktx_read.wasm` : p),
            }).then((ktx) => {
                window.ktx = ktx;
                ktxReady.resolve();
            });
    ...
    // device.
    _configureContext(device: GPUDevice): void {
        this.gpu = this.canvas.getContext('webgpu') as GPU;
    ...
    _onDeviceLost(info: { reason: string; message: string }): void {
        console.error(`WebGPU device was lost: ${info.message} (reason: ${info.reason})`);
        this.isReady = false;
        this._deviceLost = true;
        this.deviceLostHandlers.forEach((cb) => cb?.(info as any));
    ...
    _restore(device: GPUDevice): void {
        this._configureContext(device);
    ...
    // an unsubscribe fn.
    addResizeHandler(cb: (w: number, h: number) => void): () => void {
    ...
    add(f: (args: UpdateArgs) => void): void {
        this.callBacks.add(f);
    }

    remove(f: (args: UpdateArgs) => void): void {
    ...
    setClearColor({ r = 0, g = 0, b = 0, a = this.transparent ? 0 : 1 }: Partial<ClearColor> = {}): void {
    ...
    trackCompile(promise: Promise<any>): void {
        if (!promise) return;
    ...
    getRenderQueue({ scene, camera, sort = true, frustumCull = true }: { scene: Transform; camera?: Camera; sort?: boolean; frustumCull?: boolean } = {} as any): Mesh[] {
        this.renderQueue = [];
    ...
    render(
        ...
        if (target) {
            ...
            target.textures.forEach((texture, i) => {
                ...
                colorAttachments.push(colorAttachment as GPURenderPassColorAttachment);
            });
            ...
            if (!target.depthTexture) {
                ...
                renderPassDescriptor = {
                    ...
                    depthStencilAttachment: {
                        view: target?.depthTexture?.createView?.() as GPUTextureView,
                        depthClearValue: 1.0,
                        depthLoadOp,
                        depthStoreOp,
                    },
            ...
            renderPassDescriptor = {
                colorAttachments: [
                    {
                        view: this?.gpu?.getCurrentTexture()?.createView?.() as GPUTextureView,
