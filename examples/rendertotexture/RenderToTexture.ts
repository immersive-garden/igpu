...
export class RenderToTexture {
    ...
    scene!: Transform;
    sceneOne!: Transform;
    sceneBuffer!: RenderTarget;
    display!: Mesh;
    displayPipeline!: RenderPipeline;
    displaySampler!: GPUSampler;
    testMesh!: Mesh;
    testMeshTwo!: Mesh;
            ...
            bindGroups: (uniformBuffer) => [
                this.gpu.device.createBindGroup({
                    ...
                    entries: [
                        ...
                        {
                            binding: (pipeline.defs as any).samplers.sampler2d.binding,
                            resource: this.displaySampler,
                        },
                        {
                            binding: (pipeline.defs as any).textures.map.binding,
                            resource: this.sceneBuffer.createView(0),
                        },
                        {
                            binding: (pipeline.defs as any).textures.normals.binding,
    ...
    initTestScene() {
        this.sceneBuffer = new RenderTarget(
            this.gpu,
            {
                ...
                format: 'bgra8unorm',
                width: (this.gpu.canvas as HTMLCanvasElement).width,
                height: (this.gpu.canvas as HTMLCanvasElement).height,
                depthTexture: true,
                sampleCount: 4,
            },
            [
    ...
    handleResize = () => {
        ...
        this.display.bindGroups[0] = this.gpu.device.createBindGroup({
            ...
            entries: [
                ...
                {
                    binding: (this.displayPipeline.defs as any).samplers.sampler2d.binding,
                    resource: this.displaySampler,
                },
                {
                    binding: (this.displayPipeline.defs as any).textures.map.binding,
                    resource: this.sceneBuffer.createView(0),
                },
                {
                    binding: (this.displayPipeline.defs as any).textures.normals.binding,
                    resource: this.sceneBuffer.createView(1),
                },
            ],
        });
