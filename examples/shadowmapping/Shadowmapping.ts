...
export class Shadowmapping {
    ...
    async init(el: HTMLElement | null) {
        ...
        const airplaneUV = Float32Array.from(jsonData.uv, (val: number, i: number) => (i % 2 ? 1 - val : val));
        ...
        const shadowView = makeStructuredView((renderPipeline.defs as any).uniforms.shadowUniforms);

        const shadowUniformBuffer = createUniformBuffer(this.gpu, {
            label: 'shadow-uniform-buffer',
            size: (shadowViewProjectionMatrix as any).byteLength,
        });
        ...
        const makeRenderBG = (uniformBuffer: GPUBuffer, gpuTexture: GPUTexture) =>
