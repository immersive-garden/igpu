...
export class Texture {
        ...
        if (src !== null && src !== undefined) {
            ...
            if (isUrl) {
                if (srcArray.length === 1) {
                    loadPromise = createTextureFromImage(device, srcArray[0] as string, opts as any);
                } else {
                    loadPromise = createTextureFromImages(device, srcArray as string[], opts as any);
                }
            } else {
                if (srcArray.length === 1) {
                    loadPromise = Promise.resolve(createTextureFromSource(device, srcArray[0] as any, opts as any));
                } else {
                    loadPromise = Promise.resolve(createTextureFromSources(device, srcArray as any[], opts as any));
                }
            }
    ...
    update({
        ...
        sampleCount = 1,
        mipLevelCount = 1,
    }: Omit<TextureOptions, 'src' | 'flipY' | 'mips' | 'isCubeMap' | 'label' | 'generateMipmaps'> = {}): void {
        const needsInit =
            width !== this.width ||
            height !== this.height ||
            depth !== this.depth ||
        ...
        if (data && !isRenderAttachment && this.texture) {
            ...
            let _data: (ArrayBufferView | null)[] | null = this.data instanceof Array ? this.data : [this.data as ArrayBufferView];
