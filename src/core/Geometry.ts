...
import type { GPU, Bounds, GeometryData } from '@/types';
...
export class Geometry {
    ...
    constructor(gpu: GPU, { data, instancedData, interleave = false, drawBuffer = null }: GeometryOptions = {}) {
        ...
        this.nonInstancedVerts = createBuffersAndAttributesFromArrays(gpu.device, data as any);
        ...
        this.instancedVerts = instancedData ? createBuffersAndAttributesFromArrays(gpu.device, instancedData as any, instanceOptions as any) : {};
    ...
    _positionAttr(): PositionAttr | null {
        const pos = (this.attributes as any)?.position;
        if (!pos) return null;
    ...
    computeBoundingBox(attr: PositionAttr | null = this._positionAttr()): Bounds | null {
        if (!attr) return null;
    ...
    computeBoundingSphere(attr: PositionAttr | null = this._positionAttr()): Bounds | null {
        if (!attr) return null;
