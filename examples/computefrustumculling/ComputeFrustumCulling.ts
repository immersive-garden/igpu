...
export class ComputeFrustumCulling {
    ...
    cullData!: ArrayBuffer;
    cullF32!: Float32Array;
    cullUniform!: GPUBuffer;
    cull!: ComputeShader;
    cullKernel!: any;
    ...
    async _initField() {
        ...
        this.vertexCount = (geometry as any).nonInstancedVerts.numElements;
        ...
        for (let i = 0; i < NUM; i++) {
            ...
            tmp.scale.set(s, s, s);
            tmp.updateMatrix();
            models.set(tmp.matrix as any, i * 16);

            center.copy(localCenter).applyMat4(tmp.matrix as any);
            spheres[i * 4 + 0] = center.x;
            spheres[i * 4 + 1] = center.y;
            spheres[i * 4 + 2] = center.z;
            spheres[i * 4 + 3] = localRadius * (tmp.matrix as any).getMaxScaleOnAxis();
        }
        ...
        (geometry as any).drawBuffer = this.argsBuffer;
    ...
    _runCull() {
        const frustum = (this.frustumCamera as any).frustum as Array<{ x: number; y: number; z: number; constant: number }>;
        for (let i = 0; i < 6; i++) {
            const p = frustum[i];
