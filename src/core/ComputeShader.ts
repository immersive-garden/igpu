...
export class ComputeShader {
    ...
    defs!: any;
    _buildOpts!: { layout: GPUPipelineLayout | 'auto'; constants: Record<string, number> };
    _pipelineLayouts!: Record<string, GPUPipelineLayout>;
    _bindGroupLayouts!: Record<string, GPUBindGroupLayout[]>;
    _unregister!: () => void;
    querySet!: GPUQuerySet;
    queryBuffer!: GPUBuffer;
    queryBufferResult!: GPUBuffer;

    constructor(gpu: GPU, { label = '', code = ``, layout = 'auto' as GPUPipelineLayout | 'auto', constants = {}, size = 0 }: ComputeShaderOptions = {} as any) {
        if (!gpu) {
    ...
    _resolveLayout(key: string, layout: GPUPipelineLayout | 'auto'): GPUPipelineLayout {
        ...
        if (!this._pipelineLayouts[key]) {
            ...
            const bindGroupLayouts = descriptors.map((d: any) => device.createBindGroupLayout(d));
    ...
    isValidKernel(key: string): boolean {
        const keys: string[] = [];
        const { entryPoints } = this.defs;
        for (const _key in entryPoints) keys.push(_key);
    ...
    bindGroupLayout(kernelOrKey: GPUComputePipeline | string, groupIndex = 0): GPUBindGroupLayout {
    ...
    async dispatch(encoder: GPUCommandEncoder, { pass = null, kernel, bindGroup, bindGroupIndex = 0, dispatchCount, workgroupBuffer = null, timing = false }: DispatchOptions): Promise<void> {
        if (!encoder) {
        ...
        if (workgroupBuffer) {
            ...
            _pass.dispatchWorkgroups(...(dispatchCount as [number, number?, number?]));
        }
