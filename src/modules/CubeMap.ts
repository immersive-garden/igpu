...
type CubeMapSrc = string | ImageBitmap | HTMLImageElement | HTMLCanvasElement | OffscreenCanvas | VideoFrame;
...
export class CubeMap {
    ...
    constructor(
        gpu: GPU,
        { src = [], mips = false, flipY = false, usage = GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST | GPUTextureUsage.RENDER_ATTACHMENT, label = 'CubeMap' }: CubeMapOptions = {}
    ) {
        if (src.length !== 6) {
            throw new Error(`CubeMap: expected 6 face sources, got ${src.length}`);
        }

        this._texture = new Texture(gpu, { src: src as any, mips, flipY, usage, label, isCubeMap: true });
