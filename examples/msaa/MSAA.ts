...
export class MSAA {
    ...
    scene!: Transform;
    geometry!: Plane;
    leafSampler!: GPUSampler;
    leafView!: GPUTextureView;
    color!: Color;
    target!: RenderTarget;
    leaf!: Mesh;
    leafScene!: Transform;
    ...
    async init() {
        ...
        gui.add(this.params, 'sampleCount', { label: 'sample-count', min: 1, max: 4, step: 3 }).on('change', ({ value }: { value: number }) => this.buildTarget(value));
    ...
    handleResize = () => {
        ...
        this.target.onResize({ width: (this.gpu.canvas as HTMLCanvasElement).width, height: (this.gpu.canvas as HTMLCanvasElement).height });
        this.bindDisplay();
    };
}
