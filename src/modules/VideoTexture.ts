...
export class VideoTexture {
    ...
    ready: Promise<this>;
    _callbackHandle: number | null;
    _useRVFC: boolean;
    _running: boolean;
    _metadataReady: boolean;
    _videoWidth: number;
    _videoHeight: number;
    _ownedVideo: boolean;
    _resolveReady: ((value: this) => void) | null;

    constructor(gpu: GPU, { video, format = 'rgba8unorm', label = '', autoStart = true, flipY = false }: VideoTextureOptions) {
        this.gpu = gpu;
        this.id = TEXTURE_ID++;
        ...
        this.ready = new Promise<this>((resolve) => {
    ...
    _schedule(): void {
        ...
        if (this._useRVFC) {
            this._callbackHandle = (this.video as any).requestVideoFrameCallback(this._tick);
        } else {
    ...
    _cancelPending(): void {
        ...
        if (this._useRVFC) {
            (this.video as any).cancelVideoFrameCallback(this._callbackHandle);
        } else {
