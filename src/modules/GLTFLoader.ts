...
export class GLTFLoader {
    ...
    async load(url: string): Promise<Transform> {
        ...
        this.buffers = await Promise.all((json.buffers || []).map((b: any, i: any) => loadBuffer(b, baseUrl, glbBinary, i)));
    ...
    async _buildNode(index: number, parent: Transform): Promise<void> {
        ...
        if (node.matrix) {
            ...
            transform.matrix = new Float32Array(node.matrix) as any as Mat4;
        } else {
            ...
            if (node.rotation) transform.quaternion.set(...(node.rotation as [number, number, number, number]));
    ...
    async _buildMesh(meshIndex: number, parent: Transform, skinIndex: number | undefined): Promise<void> {
    ...
    async _buildPrimitive(primitive: any, name = 'gltf', skinIndex: number | undefined, parent: Transform): Promise<Mesh | null> {
        ...
        if (skinned) {
            (m as any).skinned = true;
            (m as any).skin = skinIndex;
    ...
    _readAttribute(accessorIndex: number): Float32Array {
        // vertex attributes are normalized to packed Float32 for the float vertex layout
        return readAccessor(this.json, this.buffers, accessorIndex, true) as Float32Array;
    }

    _readIndices(accessorIndex: number): Uint16Array | Uint32Array {
    ...
    _readJoints(accessorIndex: number): Uint32Array {
        const raw = readAccessor(this.json, this.buffers, accessorIndex, false);
    ...
    _buildSkins(): void {
        ...
        this.json.nodes.forEach((n: any, i: any) =>
            (n.children || []).forEach((c: any) => {
                parentOf[c] = i;
            })
        );

        this.skins = skins.map((skin: any) => {
            const joints = skin.joints;
            const nodeToBone = new Map();
            joints.forEach((nodeIdx: any, bone: any) => nodeToBone.set(nodeIdx, bone));
            ...
            joints.forEach((nodeIdx: any, bone: any) => {
    ...
    _buildAnimations(): void {
        const anims = this.json.animations || [];
        this.animations = anims.map((anim: any, ai: any) => {
            let duration = 0;
    ...
    getSkinData(meshOrIndex: number | any = 0): any {
    ...
    async getSkinnedMesh({
        ...
        label = 'skinned',
        material = {},
    }: { code?: string; ibl?: any; mesh?: number; animation?: number; fps?: number; label?: string; material?: Record<string, any> } = {}): Promise<{ mesh: Mesh; skin: any; animation: any } | null> {
    ...
    getGeometryData(meshOrIndex: number | any = 0): any {
    ...
    getGeometry(meshOrIndex: number | any = 0): Geometry | null {
        ...
        const geometry = new Geometry(this.gpu, { data });
        (geometry as any).hasTangents = d.hasTangents;
        return geometry;
    }
    ...
    getAnimation({ animation = 0, skin = 0, fps = 30 }: { animation?: number; skin?: number; fps?: number } = {}): any {
        ...
        if (ac) sa.nodes.forEach((nd: any, a: any) => nodeToPose.set(nd, a));
        sk.nodeToBone.forEach((bone: any, nd: any) => nodeToPose.set(nd, ac + bone));

        const chans = anim.channels.map((c: any) => ({ ...c, target: nodeToPose.get(c.node) })).filter((c: any) => c.target !== undefined);

        if (chans.length === 0) {
            const sampleNodes = anim.channels.slice(0, 3).map((c: any) => c.node);
...
function tangentAttribute(tangent: Float32Array | null, vertexCount: number): { data: Float32Array; numComponents: number } {
...
function localTRS(node: any): { t: number[]; r: number[]; s: number[] } {
    if (node.matrix) {
...
function findSeg(times: ArrayLike<number>, t: number): [number, number, number] {
...
function sampleVec(c: any, t: number, stride: number): number[] {
...
function sampleQuat(c: any, t: number): number[] {
    const cubic = c.interp === 'CUBICSPLINE';
...
function parseGLB(buf: ArrayBuffer): { json: any; binary: ArrayBuffer | null } {
...
async function loadBuffer(bufferDef: any, baseUrl: string, glbBinary: ArrayBuffer | null, index: number): Promise<ArrayBuffer> {
...
function readAccessor(json: any, buffers: ArrayBuffer[], accessorIndex: number, toFloat: boolean): Float32Array | Int8Array | Uint8Array | Int16Array | Uint16Array | Uint32Array {
    const accessor = json.accessors[accessorIndex];
    const comp = (COMPONENT as any)[accessor.componentType];
    const numComponents = (TYPE_COMPONENTS as any)[accessor.type];
...
function dataViewReader(view: DataView, componentType: number): (o: number) => number {
...
function normalize(v: number, componentType: number): number {
    switch (componentType) {
        case 5120:
...
function computeNormals(positions: Float32Array, indices: Uint16Array | Uint32Array | null): Float32Array {
