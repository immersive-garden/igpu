...
// glTF rig + animation skinning, PBR-lit (metallic-roughness + IBL), with a
// real-time shadow map (depth pass + PCF) the character casts onto a floor and
// itself. The JSON path lives in examples/skinning.
export class SkinningGLTF {
    ...
    async init(canvas: HTMLElement | null) {
        ...
        const cv = this.gpu.canvas as HTMLCanvasElement;
        ...
        this.orbit = new Orbit(this.camera, { element: cv, target: [0, 1, 0] });

        this.addCredit('Skinning (glTF) + PBR + shadows. Animation from <a href="https://www.mixamo.com" target="_blank">Mixamo</a>.');
        ...
        const loader: any = new GLTFLoader(this.gpu, { dataOnly: true });
        ...
        const lightPos: [number, number, number] = [4, 8, 4];
        const lightTarget: [number, number, number] = [0, 1, 0];
        ...
        const shadowView = makeStructuredView((pipeline as any).defs.uniforms.shadowUniforms);
        ...
        const shadowDepthView = (this.shadowBuffer as any).depthTexture.createView();

        const materialView = makeStructuredView((pipeline as any).defs.uniforms.material);
        materialView.set({
