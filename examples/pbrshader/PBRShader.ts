...
interface CarMaps {
    ...
    rmoView: GPUTextureView;
    normalView: GPUTextureView;
    emissiveView: GPUTextureView;
    opacityView: GPUTextureView;
...
export class PBRShader {
    ...
    async init(el: HTMLElement | null) {
        const canvas = (el || document.getElementById('web-gpu-canvas')) as HTMLCanvasElement | null;
    ...
    async initTestScene() {
        ...
        this.orbit = new Orbit(this.camera, { element: this.gpu.canvas as HTMLCanvasElement });
    ...
    async swizzleRMO(url: string): Promise<GPUTexture> {
    ...
    async addCarPart(jsonUrl: string, maps: CarMaps, { transparent = false } = {}): Promise<Mesh> {
        ...
        const materialView: StructuredView = makeStructuredView((pipeline.defs as any).uniforms.material);
        materialView.set({
            ...
            alphaMode: 0,
            hasNormalMap: 1,
            hasTangents: (geometry as any).hasTangents ? 1 : 0,
        ...
        (mesh as any).material = { view: materialView, buffer: materialBuffer };
    ...
    initProbes(ibl: IBLResult) {
        ...
        const makeProbe = (mode: number, position: Vec3) => {
            const pipeline = new RenderPipeline(this.gpu, {
    ...
    async initIBL({ url = './assets/pbr/artistworkshop_oct.exr', shUrl = './assets/pbr/artistworkshop_sh.json' } = {}): Promise<IBLResult> {
    ...
    solidTexture(rgba: number[], label: string): GPUTexture {
        const texture = this.gpu.device.createTexture({
            size: [2, 2],
            format: 'rgba8unorm',
    ...
    initPane() {
        ...
        const apply = () => {
            for (const mesh of this.carMeshes) {
                const mat = (mesh as any).material as { view: StructuredView; buffer: GPUBuffer };
                mat.view.set({
                    roughnessFactor: this.params.roughnessFactor,
                    metallicFactor: this.params.metallicFactor,
                    occlusionStrength: this.params.occlusionStrength,
                ...
                this.gpu.device.queue.writeBuffer(mat.buffer, 0, mat.view.arrayBuffer);
            }
        };
        ...
        (this.pane as any)
            ...
            .on('change', apply);
        (this.pane as any).addBinding(this.params, 'baseColor', { label: 'albedo', color: { type: 'float' } }).on('change', apply);

        (this.pane as any).addBinding(this.params, 'useGeometricNormal', { label: 'geo-normals' }).on('change', apply);

        (this.pane as any).addBinding(this, 'showProbes', { label: 'ibl-probes' }).on('change', (ev: any) => {
            this.probes.visible = ev.value;
        });
    }

    update = ({ time = 0, deltaTime = 0 }: { time?: number; deltaTime?: number } = {}) => {
        this.orbit.update();
