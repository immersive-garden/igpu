...
const BLOCK_INFO: Record<string, [number, number, number]> = {
...
export const formatBlockInfo = (format: string): BlockInfo | null => {
    const e = BLOCK_INFO[format];
    return e ? { blockW: e[0], blockH: e[1], blockBytes: e[2] } : null;
};

export const parseKTXHeader = (u8: Uint8Array): KTXHeader => {
...
const VK_TO_WEBGPU: Record<number, string> = {
    // 8-bit
...
export const vkFormatToWebGPU = (fmt: number): string | null => VK_TO_WEBGPU[fmt] ?? null;

// GL internal format → WebGPU format string.
const GL_TO_WEBGPU: Record<number, string> = {
    // 8-bit
...
export const glFormatToWebGPU = (fmt: number): string | null => GL_TO_WEBGPU[fmt] ?? null;

// 16-bit VK format range: R16_UNORM(70) … R16G16B16A16_SFLOAT(97)
const isVkFormat16Bit = (fmt: number): boolean => fmt >= 70 && fmt <= 97;
...
const isGLFormat16Bit = (fmt: number): boolean => GL_16BIT.has(fmt);

const isVkFormatSRGB = (fmt: number): boolean => {
...
const isGLInternalFormatSRGB = (fmt: number): boolean => {
    const sRGBFormats = new Set([
        0x8c41, // GL_SRGB
