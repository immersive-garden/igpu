...
export class Skin {
    ...
    initBones(): void {
        ...
        if (ancestors && ancestors.count) {
            let parentTransform: Transform = this.root;
        ...
        this.rig.bones.forEach((bone: any, i: number) => {
        ...
        if (this.rig.inverseBindMatrices) {
            ...
            this.bones.forEach((bone: any, i: number) => {
    ...
    initSkinning(): void {
        ...
        this.threadCount = (position as any).length / 3;
        const positionData = new Float32Array(position as ArrayLike<number>);
        const normalData = new Float32Array(normal as ArrayLike<number>);
        const weightData = new Float32Array(weight as ArrayLike<number>);
        const boneIndexData = new Uint32Array(boneIndex as ArrayLike<number>);
