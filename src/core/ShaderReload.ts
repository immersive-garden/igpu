...
interface ShaderInstance {
    code: string;
    reload: (newCode: string) => void;
}

declare global {
    var __shaderRegistry: Set<ShaderInstance> | undefined;
}

const noop = (): void => {};

export function registerShader(instance: ShaderInstance): () => void {
...
if (import.meta.hot) {
    ...
    globalThis.__reloadShader = (oldCode: string, newCode: string): void => {
