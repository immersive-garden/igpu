...
interface RaycastBoundsOptions {
    maxDistance?: number;
    output?: Mesh[];
}
...
function attrData(attr: any): ArrayLike<number> | null | undefined {
    return attr?.data ?? attr;
}

function attrStride(attr: any, fallback: number): number {
...
export class Raycast {
    ...
    castMouse(camera: Camera, mouse: [number, number] = [0, 0]): this {
        if (camera.type === 'orthographic') {
            // Orthographic ray origin is on the near plane, not the camera position
            const { left = 0, right = 0, bottom = 0, top = 0, zoom } = camera;
    ...
    intersectBounds(meshes: Mesh | Mesh[], { maxDistance, output = [] }: RaycastBoundsOptions = {}): Mesh[] {
    ...
    intersectMeshes(meshes: Mesh | Mesh[], { cullFace = true, maxDistance, includeUV = true, includeNormal = true, output = [] }: RaycastIntersectOptions = {}): Mesh[] {
    ...
    intersectPlane(plane: Plane, origin = this.origin, direction = this.direction, out: Vec3 | null = null): number {
    ...
    intersectSphere(sphere: Pick<Bounds, 'center' | 'radius'>, origin = this.origin, direction = this.direction): number {
    ...
    intersectBox(box: Pick<Bounds, 'min' | 'max'>, origin = this.origin, direction = this.direction): number {
        let tmin, tmax;
        ...
        const tYmin = ((invdiry >= 0 ? min.y : max.y) - origin.y) * invdiry;
        const tYmax = ((invdiry >= 0 ? max.y : min.y) - origin.y) * invdiry;
        if (tmin > tYmax || tYmin > tmax) return 0;
        if (tYmin > tmin) tmin = tYmin;
        if (tYmax < tmax) tmax = tYmax;
        const tZmin = ((invdirz >= 0 ? min.z : max.z) - origin.z) * invdirz;
        const tZmax = ((invdirz >= 0 ? max.z : min.z) - origin.z) * invdirz;
